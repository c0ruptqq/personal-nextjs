...
const Navbar = () => {
  ...
  const CustomLink = ({ href, title, className = "", click }) => {
    return (
      <Link href={href} className={`${className} relative group text-2xl flex justify-center text-center ${currentRoute === href ? "text-primary" : "text-text"}`} onClick={click}>
        {title}
        <span className="h-[3px] inline-block w-0 bg-white absolute left-0 -bottom-0.5 lg:group-hover:w-full transition-[width] ease duration-300"></span>
      </Link >
    )
  ...
  return (
    <div className={`flex justify-center content-center items-center absolute w-screen ${height_class} lg:items-start lg:h-fit`}>
      <Link
        href="/" className="h-40 aspect-square absolute border-none z-30 top-[-0.5em] cursor-pointer"><Logo /></Link>
      ...
      <div
        className={`flex flex-col justify-center items-center content-center min-w-[90vw] min-h-[100vh] rounded-2xl z-10 bg-bg/40 backdrop-blur-md ${menu_class} duration-300 lg:expanded lg:bg-inherit lg:flex lg:relative lg:min-h-fit lg:w-screen lg:flex-row`}>
        <ul
